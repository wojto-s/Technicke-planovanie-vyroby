...
export function VyrobaPopup({ addVyroba, tableIndex, vyroba, currentID }) {
  ...
  const handleVyrobokChange = (e) => {
    const vyrobokChange = e.target.value;
    setCurrentCisloVykresu(vyrobokChange);
    const firstStroj = availableStroj(vyrobokChange);
    setNewStroj(firstStroj[0]);
  };
          ...
          <div className="d-flex mb-3">
            <div>
              <label className="d-flex col-6 justify-content-between mb-2">
                Názov:
                <select className="ms-4" onChange={handleVyrobokChange}>
                  {vyrobky.map((vyrobok) => (
